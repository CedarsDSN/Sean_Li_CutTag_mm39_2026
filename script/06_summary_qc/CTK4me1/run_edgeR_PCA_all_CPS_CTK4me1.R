...
out_dir <- "/common/lix5lab/Li_Xue_Cut_Tag_06162022/Analysis_WYZ/mm39/PCA_edgeR/CTK4me1"
dir.create(out_dir, showWarnings = FALSE, recursive = TRUE)

all_matrix_files <- list.files(
  quant_dir,
  pattern = "^CPS[0-9]+_seacr_consensus_profile_bins\\.xls$",
  full.names = TRUE
)
...
for (mat_file in all_matrix_files) {
  ...
  cps_id <- sub(".*(CPS[0-9]+)_seacr_consensus_profile_bins\\.xls$", "\\1", basename(mat_file))
  message(paste("\nProcessing:", cps_id))
