...
RESULT_DIR="${PROJECT_ROOT}/edgeR_DE_results/CTK27ac"
QUANT_DIR="${PROJECT_ROOT}/quantification/CTK27ac"
MANIFEST_DIR="${QUANT_DIR}/manifests"
PCA_DIR="${PROJECT_ROOT}/PCA/CTK27ac"
...
# move aggregate outputs into Summary folder
...
# move edgeR QC plots if present
...
#    quantification/CTK27ac/CPSX/
# --------------------------------------------------
if [[ -d "$QUANT_DIR" ]]; then
    for f in "${QUANT_DIR}"/CPS*_macs3_per_sample_summit_profile_bins.xls \
             "${QUANT_DIR}"/CPS*_macs3_per_sample_summit_profile_bins_log.txt; do
        [[ -e "$f" ]] || continue
        fname=$(basename "$f")
        cps=$(echo "$fname" | sed -E 's/^(CPS[0-9]+)_macs3_per_sample_summit_profile_bins(_log\.txt|\.xls)/\1/')
...
if [[ -d "$MANIFEST_DIR" ]]; then
    for f in "${MANIFEST_DIR}"/CPS*_CTK27ac_manifest.tsv; do
        [[ -e "$f" ]] || continue
        fname=$(basename "$f")
        cps=$(echo "$fname" | sed -E 's/^(CPS[0-9]+)_CTK27ac_manifest\.tsv/\1/')
        mkdir -p "${QUANT_DIR}/${cps}"
        mv "$f" "${QUANT_DIR}/${cps}/"
    done
